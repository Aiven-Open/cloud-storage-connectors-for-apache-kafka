...
public abstract class OutputWriter implements AutoCloseable {
    ...
    public static class Builder {
        ...
        public OutputWriter build(final FormatType formatType) throws IOException {
            ...
            switch (formatType) {
                case CSV:
                    return PlainOutputWriter.createFor(outputFields, getCompressedStream(outputStream));
                case JSONL:
                    return JsonLinesOutputWriter.createFor(outputFields, getCompressedStream(outputStream));
                case JSON:
                    return JsonOutputWriter.createFor(outputFields, getCompressedStream(outputStream));
                default:
