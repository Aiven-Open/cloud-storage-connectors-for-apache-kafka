...
final public class S3SinkConfig extends AivenCommonConfig {
    ...
    private static final String GROUP_FORMAT = "Format"; // NOPMD UnusedPrivateField
    ...
    private static void addAwsConfigGroup(final ConfigDef configDef) {
        ...
        configDef.define(AWS_S3_REGION_CONFIG, Type.STRING, null, new AwsRegionValidator(), Importance.MEDIUM,
                "AWS S3 Region, e.g. us-east-1", GROUP_AWS, awsGroupCounter++, ConfigDef.Width.NONE,
    ...
    private static void addAwsStsConfigGroup(final ConfigDef configDef) {
        ...
        configDef.define(AWS_STS_ROLE_ARN, ConfigDef.Type.STRING, null, new ConfigDef.NonEmptyString(),
                Importance.MEDIUM, "AWS STS Role", GROUP_AWS_STS, awsStsGroupCounter++, // NOPMD UnusedAssignment
    ...
    private static void addFileConfigGroup(final ConfigDef configDef) {
        AivenCommonConfig.addFileConfigGroup(configDef, GROUP_FILE, "S3", 0, null);
    }
        ...
        if (awsStsRole.isValid()) {
            ...
            if (!awsNewSecret.isValid()) {
                ...
                if (awsOldSecret.isValid()) {
                    LOGGER.error("Config options {} and {} are deprecated", AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY);
                } else {
                    LOGGER.info(
