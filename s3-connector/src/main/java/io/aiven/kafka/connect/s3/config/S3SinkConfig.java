        ...
        if (awsStsRole.isValid()) {
            ...
            if (!awsNewSecret.isValid()) {
                ...
                if (awsOldSecret.isValid()) {
                    LOGGER.error(String.format("Config options %s and %s are deprecated", AWS_ACCESS_KEY_ID,
                            AWS_SECRET_ACCESS_KEY));
                } else {
                    throw new ConfigException(
                            String.format("Either {%s, %s} or {%s, %s} should be set", AWS_ACCESS_KEY_ID_CONFIG,
                                    AWS_SECRET_ACCESS_KEY_CONFIG, AWS_STS_ROLE_ARN, AWS_STS_ROLE_SESSION_NAME));
                }
            }
        }
