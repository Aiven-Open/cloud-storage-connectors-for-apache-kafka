...
public abstract class AbstractAvroIntegrationTest<N, K extends Comparable<K>> extends AbstractSinkIntegrationTest<N, K> {
    ...
    void avroOutput(final String avroCodec, final CompressionType compression)
        ...
        final K[] expectedBlobs = arrayOf(sinkStorage.getAvroBlobName(prefix, topicName, 0, 0, compression),
                sinkStorage.getAvroBlobName(prefix, topicName, 1, 0, compression), sinkStorage.getAvroBlobName(prefix, topicName, 2, 0, compression),
                sinkStorage.getAvroBlobName(prefix, topicName, 3, 0, compression));

//        List<K> keys = Collections.emptyList();
//        StopWatch stopwatch = StopWatch.createStarted();
//        while (keys.size() < expectedBlobs.length) {
//            keys = this.getNativeKeys();
//        }
//        stopwatch.stop();
        ...
        for (final K nativeKey : expectedBlobs) {
            for (final GenericRecord r : AvroTestDataFixture
                    .readAvroRecords(readBytes(nativeKey, compression))) {
    ...
    void jsonlAvroOutputTest() throws ExecutionException, InterruptedException, IOException {
        ...
        // get list of expected blobs
        final K[] expectedBlobs = arrayOf(sinkStorage.getBlobName(prefix, topicName, 0, 0, compression),
    ...
    private List<GenericRecord> produceRecords(final int recordCountPerPartition, final int partitionCount,
                                               final String topicName, Function<Integer, GenericRecord> func) throws ExecutionException, InterruptedException {
        final List<Future<RecordMetadata>> sendFutures = new ArrayList<>();
        final List<GenericRecord> genericRecords = AvroTestDataFixture
...
//    }
    ...
    void defaultAvroOutput() throws ExecutionException, InterruptedException, IOException {
        ...
        CompressionType compression = sinkStorage.getDefaultCompression();
        final Map<String, String> connectorConfig = createConfiguration(topicName);
        OutputFormatFragment.setter(connectorConfig).withFormatType(FormatType.AVRO)
                .withOutputFields(OutputFieldType.KEY, OutputFieldType.VALUE);
        ...
        List<GenericRecord> expectedGenericRecords = produceRecords(recordCountPerPartition, partitionCount, topicName);
        ...
        final List<String> expected = expectedGenericRecords.stream()
                .map(mapF("message")).collect(Collectors.toList());
...
//                        + "\"connect.version\":1,\"connect.name\":\"input_data\"}");
        ...
        enrichedValues.addAll(AvroTestDataFixture
                .generateAvroRecords(recordCountPerPartition * partitionCount, i -> AvroTestDataFixture.generateAvroRecord(i, AvroTestDataFixture.CONNECT_EXTRA_SCHEMA)));
...
//        }
    ...
    void schemaChanged() throws ExecutionException, InterruptedException, IOException {
        ...
        List<GenericRecord> expectedRecords = produceRecords(3, 1, topicName, i -> {
            if (i % 2 == 0) {
                GenericRecord rec = AvroTestDataFixture.generateAvroRecord(i, AvroTestDataFixture.EVOLVED_SCHEMA);
                rec.put("age", i);
                return rec;
            }
        ...
        for (final K blobName : expectedBlobs) {
            AvroTestDataFixture.readAvroRecords(readBytes(blobName, compression)).forEach(genericRecord -> actualValues.add(genericRecord.toString()));
        }

        List<String> expectedValues = expectedRecords.stream().map(GenericRecord::toString).collect(Collectors.toList());
    ...
    void jsonlOutput() throws ExecutionException, InterruptedException, IOException {
        ...
        final Map<String, String> connectorConfig = createConfiguration(topicName);
        OutputFormatFragment.setter(connectorConfig).withFormatType(FormatType.JSONL)
                .withOutputFields(OutputFieldType.KEY, OutputFieldType.VALUE)
                        .withOutputFieldEncodingType(OutputFieldEncodingType.NONE);
        FileNameFragment.setter(connectorConfig).fileCompression(compression);
...
//
//        final Map<String, String> config = testData.createConnectorConfig(null);;
