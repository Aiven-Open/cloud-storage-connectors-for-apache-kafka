...
final class OffsetManagerTest {
    ...
    @Test
    void testIncrementAndUpdateOffsetExistingOffset() {
        sourceTaskContext = mock(SourceTaskContext.class);
        final OffsetStorageReader offsetStorageReader = mock(OffsetStorageReader.class);
        when(sourceTaskContext.offsetStorageReader()).thenReturn(offsetStorageReader);
        ...
        final Map<String, Object> offsetValue = new HashMap<>();
        offsetValue.put(OffsetManager.getOffsetKey(objectKey), 1L); // Existing offset value
        final Map<Map<String, Object>, Map<String, Object>> offsets = new HashMap<>();
        offsets.put(partitionKey, offsetValue);
        ...
        final long newOffset = offsetManager.incrementAndUpdateOffset(partitionKey, objectKey, 2L);

        assertThat(newOffset).isEqualTo(2L); // Expect incremented offset
        assertThat(offsetManager.getOffset(partitionKey, objectKey)).isEqualTo(2L); // Verify updated
        // offset in map
    }

    @Test
    void testIncrementAndUpdateOffsetNonExistingOffset() {
        sourceTaskContext = mock(SourceTaskContext.class);
        final OffsetStorageReader offsetStorageReader = mock(OffsetStorageReader.class);
        when(sourceTaskContext.offsetStorageReader()).thenReturn(offsetStorageReader);
        ...
        final long newOffset = offsetManager.incrementAndUpdateOffset(partitionKey, "", startOffset);
    ...
    @Test
    void testIncrementAndUpdateOffsetMultipleTimes() {
        sourceTaskContext = mock(SourceTaskContext.class);
        final OffsetStorageReader offsetStorageReader = mock(OffsetStorageReader.class);
        when(sourceTaskContext.offsetStorageReader()).thenReturn(offsetStorageReader);
        ...
        // Invoke method and assert new offset value
        final long startOffset = 5L;
        long newOffset = offsetManager.incrementAndUpdateOffset(partitionKey, "SomeKey", startOffset);

        // Expect the startOffset to be returned when no existing offset is found
        assertThat(newOffset).isEqualTo(startOffset);

        newOffset = offsetManager.incrementAndUpdateOffset(partitionKey, "SomeKey", startOffset);
        assertThat(newOffset).isEqualTo(6L);
