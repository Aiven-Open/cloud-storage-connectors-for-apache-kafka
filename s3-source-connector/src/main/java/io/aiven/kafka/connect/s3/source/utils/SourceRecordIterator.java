...
public final class SourceRecordIterator
    ...
    @Override
    protected Stream<S3SourceRecord> getSourceRecordStream(String offset) {
        return sourceClient.getS3ObjectStream(offset).map(fileMatching).filter(taskAssignment).map(Optional::get);
    }

    @Override
    protected IOSupplier<InputStream> getInputStream(S3SourceRecord sourceRecord) {
    ...
    @Override
    protected String getName(S3Object nativeObject) {
        return nativeObject.key();
    }

    @Override
    protected S3SourceRecord createSourceRecord(S3Object nativeObject) {
        return new S3SourceRecord(nativeObject);
    }

    @Override
    protected S3OffsetManagerEntry createOffsetManagerEntry(S3Object nativeObject) {
