...
/**
 * Iterator that reads from an S3Object iterator and processes each S3Object into one or more records to be returned
 * as S3SourceRecords.
 */
public final class SourceRecordIterator implements Iterator<S3SourceRecord> {
    ...
    public static final Pattern FILE_DEFAULT_PATTERN = Pattern.compile("(?<topicName>[^/]+?)-"
            + "(?<partitionId>\\d{5})-" + "(?<uniqueId>[a-zA-Z0-9]+)" + "\\.(?<fileExtension>[^.]+)$"); // e.g. topic-00001.txt
     ...
     * @param transformer the transformer to sue.
     * @param sourceClient the source client to read from.
     */
    ...
    /**
     * Construct the OffsetManagerEntry from the S3Object file name.  This probalby should occur earlier in the chain.
     * If this method returns false the object will be skipped.
     * @param s3Object
    ...
    private boolean extractOffsetManagerEntry(final S3Object s3Object) {
        ...
        if (fileMatcher.find()) {
            ...
            final S3OffsetManagerEntry keyEntry = new S3OffsetManagerEntry(s3SourceConfig.getAwsS3BucketName(), s3Object.getKey(), fileMatcher.group(PATTERN_TOPIC_KEY),
    ...
    /**
     * Checks if the transformer should be skipped becasue the number of records exceeds what it returns.
     * This check should probalby be converted into a S3OffsetManager entry call for "isComplete" and converted into
     * a predicate check.
     *
     * @param transformer The transformer we are using.
     * @param numberOfRecsAlreadyProcessed the number of records processed.
    ...
    /**
     * Get the S3SourceRecord iterator that reads from a single object.
     * This method applies the transformer to the object and returns an iterator based on the stream
     * returned from {@link Transformer#getRecords(IOSupplier, OffsetManager.OffsetManagerEntry, AbstractConfig)}.
     * @param s3Object the object to get S3Source records from.
    ...
    private Iterator<S3SourceRecord> getS3SourceRecordIterator(final S3Object s3Object) {
        final Optional<SchemaAndValue> key = Optional.of(new SchemaAndValue(transformer.getKeySchema(), s3Object.getKey().getBytes(StandardCharsets.UTF_8)));
